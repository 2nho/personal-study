...
ALTER TABLE employee 
	ADD CONSTRAINT 
	employee_pk PRIMARY KEY employee(emp_no);
...
ALTER TABLE project
	ADD UNIQUE project_uk(start_date);
...
ALTER TABLE specialty
	ADD PRIMARY KEY(emp_no, specialty);			
 			
-- 문제90
-- assign테이블에 기본키를 추가하고 확인해보시오.(emp_no, pro_no 2개 컬럼 다 pk로 지정)			
-- 기본키명 : assign_pk	
ALTER TABLE assign
	ADD PRIMARY KEY(emp_no, pro_no);
...
ALTER TABLE assign
	ADD CONSTRAINT assing_project_fk FOREIGN KEY(pro_no) REFERENCES project(pro_no);
...
ALTER TABLE assign
	ADD CONSTRAINT assign_employee_fk FOREIGN KEY(emp_no) REFERENCES employee(emp_no);
